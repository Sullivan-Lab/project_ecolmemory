...
# # wrangling data for PDSI variable
# df3testNewPDSI <- df3testNew %>% 
#   select(Year,Month,PDSI) %>% 
#   mutate(ID=1) 
# 
# # checking whether the data is complete
# df3testNewPDSI %>% group_by(Year) %>% count() %>% filter(n!=12)
# # checking whether there is NAs --> none
# df3testNewPDSI %>% filter(is.na(PDSI))
# 
# df3testNewPDSI <- df3testNewPDSI %>% 
#   unite("year_month",Year:Month,sep="_") %>% 
#   pivot_wider(names_from = year_month,
#               values_from = PDSI) %>% 
#   slice(rep(row_number(), max(df2testNew$plot_new))) %>%  #162 equals the number of plots
...
# # wrangling data for PDSI MEAN variable
# df3testNew_MeanPDSI <- df3testNew_Mean %>% 

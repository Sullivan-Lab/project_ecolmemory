...
## Minus5y
clim.means_NoDist2 <- read_csv(file.path(workingdir,"data/L1/DataModel_CoarseEnv_FieldsAB/climateMeans/clim.means_NDist_Litter_FieldsAB_Minus5y_CoarseEnv.csv")) %>% 
    summarise(Precip_mm = sum(Precip_mm),
              MaxTemp_C = mean(MaxTemp_C)) %>% 
  mutate(exp = 1,
         dur = 2)
...
- [REMOVE ASSOCIATED CODE IF IT ISN'T INCLUDED IN THE PAPER] [SI] Cumulative weights
...
final_a <- ggdraw(a) +
  ...
  draw_label("*", x = 0.258, y = 0.710, size = 18, fontface = "bold", hjust = 0, vjust = 1, color = "#ffba76")
...
final_b <- ggdraw(b) +
  ...
  draw_label("*", x = 0.203, y = 0.720, size = 18, fontface = "bold", hjust = 0, vjust = 1, color = "#ffba76") 
 final_b
...
a<-
 ...
 final_a <- ggdraw(a) +
  ...
  draw_label("*", x = 0.280, y = 0.380, size = 18, fontface = "bold", hjust = 0, vjust = 1, color = "#ffba76")

...
ggsave(file.path(graph.wd,"Fig3_EnvCoefVSNtrt.png"), plot = last_plot(), width=11, height=8, dpi = res_export, units="in")
```
...
WEIGHTMomodelpar <- parameters_All %>% 
  ...
  mutate(ntrt = factor(rep(1:9,times=288, each=1), levels = order_levels),
         ...
         exp = ifelse(dur==1 & exp==2 & ntrt==4, 3, exp)) 
...
a<-
  ...
  scale_color_manual(values = c(pals::cols25(5)[4], pals::cols25(5)[5], "gray55"),
                     labels = c("1" = "Intact", "2" = "Disturbed", "3" = "")
                     # guide = "none"
                     ) +  
 ...
 scale_x_continuous(expand = c(0.01,0),
  ...
  theme(legend.position = "top",
        ...
        axis.title.x = element_blank(),
        axis.text.x = element_blank()) +
  guides(color = guide_legend(override.aes = list(color = c(pals::cols25(5)[4], pals::cols25(5)[5], NA))))
...
ggsave(file.path(graph.wd,"Fig4_MemoryTempPattern_LiveBiomass.png"), plot = last_plot(), width=11, height=9, dpi = res_export, units="in")
```
...
a<-
  ...
  theme_regular +
  theme(plot.margin = margin(t = 2, r = 5, b = 5, l = 5),
        title = element_text(face = "bold", size = 12))

final_a <- ggdraw(a) +
  # Rain cloud
  draw_image(image = icon_rain, x = -0.3, y = 0.46, scale = 0.073)  # Panel A
...
ggplot(aes(y=mean_mem_length,
 ...
 final_b <- ggdraw(b) +
  ...
  # Rain cloud
  draw_image(image = icon_temp, x = -0.43, y = 0.46, scale = 0.070)   # Panel A
...
```{r}
left_column <- plot_grid(final_a, final_b, ncol = 2, rel_widths = c(0.90, 1))
left_column
ggsave(file.path(graph.wd,"Fig2_DiffMemoryLength_LiveBiomass.png"), plot = last_plot(), width=7, height=4, dpi = res_export, units="in")
```
...
left_column
ggsave(file.path(graph.wd,"FigS4_DiffMemoryLength_LiveBiomass.png"), plot = last_plot(), width=7, height=8, dpi = res_export, units="in")
```
...
left_column <- plot_grid(final_a, final_b, ncol = 1)
left_column
ggsave(file.path(graph.wd,"EXTRA_FigS4_DiffMemoryLength_75_LiveBiomass.png"), plot = last_plot(), width=7, height=8, dpi = res_export, units="in")
```
...
left_column <- plot_grid(final_a, final_b, ncol = 1)
left_column
ggsave(file.path(graph.wd,"EXTRA_FigS4_DiffMemoryLength_90_LiveBiomass.png"), plot = last_plot(), width=7, height=8, dpi = res_export, units="in")
```


###- [SI] Memory Temporal Patterns
Creating datasets
```{r}
# live biomass
#dataframe
...
ggsave(file.path(graph.wd,"FigS6_MemoryTempPattern_LiveBiomass.png"), plot = last_plot(), width=11, height=9, dpi = res_export, units="in")
```
...
# saves
    ggsave(paste0(file.path(graph.wd),"/FigS7-8_MonthlyWeight_",summaryVar[Var,1],"_",summaryVar[Var,5],".png"), plot = last_plot(), width=15, height=8, dpi = res_export, units="in")
...
ggsave(file.path(graph.wd,"FigS5_StdGlobalCoefs.png"), plot = last_plot(), width=8, height=3, dpi = res_export, units="in")
```

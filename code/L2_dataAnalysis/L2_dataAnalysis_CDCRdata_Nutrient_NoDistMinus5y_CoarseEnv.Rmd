...
```{r}
workingdir <- getwd()
datain <- file.path(workingdir, "data/L1/DataModel_CoarseEnv")
graph.wd <- file.path(workingdir, "figures")
parameters.wd <- file.path(workingdir, "parameters/CoarseEnv")
```

Importing dataset
```{r}
load(file.path(datain,"DataModel_NoDistMinus5y_CoarseEnv.RData"))
...
ggplot(data = predModel, aes(y = Metric, x = Mean_pred)) +
  ...
  annotate(label = sprintf("y = %.3f + %.3f x\nR² = %.2f", coef(fit)[1], coef(fit)[2], summary(fit)$r.squared),
           geom = "text", x = 1000, y = 4000, size = 5) +

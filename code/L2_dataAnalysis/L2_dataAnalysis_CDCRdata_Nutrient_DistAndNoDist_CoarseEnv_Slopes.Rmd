...
parameters.wd <- file.path(workingdir, "parameters/CoarseEnv_Slopes/abg")
```
...
dataList <- list(
  tempData_allYears = NoDist_mu_ntrt %>% 
    filter(alpha == 3) %>% 
    bind_rows(filter(Dist_mu_ntrt, alpha == 3)) %>% 
    mutate(ntrt = rep(1:9, times = 2)) %>% 
    left_join(ntrt_qtt),

  precipData_allYears = NoDist_mu_ntrt %>% 
    filter(alpha == 2) %>% 
...
ggsave(paste0(file.path(graph.wd,paste(c("CoarseEnv_",expName, "_ModelFit_BIOMASS_.png"), collapse=""))), plot = last_plot(), width=7, height=5, dpi = 300, units="in")
...
ggsave(paste0(file.path(graph.wd,paste(c("CoarseEnv_",expName, "_ModelResiduals_.png"), collapse=""))), plot = last_plot(), width=7, height=5, dpi = 300, units="in")

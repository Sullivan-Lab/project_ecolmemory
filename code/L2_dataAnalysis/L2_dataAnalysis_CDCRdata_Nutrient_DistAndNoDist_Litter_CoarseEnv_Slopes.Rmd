...
parameters.wd <- file.path(workingdir, "parameters/CoarseEnv_FieldsAB_Slopes/litter")
```

Importing dataset
```{r}
parameters0_NoDist <- read_csv(file.path(datain,"NoDist_Litter_FieldsAB_CoarseEnv_ParametersValues.csv")) %>% 
    mutate(exp = 1)
...
parameters0_Dist <- read_csv(file.path(datain,"Dist_Litter_FieldsAB_CoarseEnv_ParametersValues.csv")) %>% 
    mutate(exp = 2)
...
ggsave(paste0(file.path(graph.wd,paste(c("CoarseEnv_Litter_",expName, "_ModelFit_BIOMASS_.png"), collapse=""))), plot = last_plot(), width=7, height=5, dpi = 300, units="in")
...
ggsave(paste0(file.path(graph.wd,paste(c("CoarseEnv_Litter_",expName, "_ModelResiduals_.png"), collapse=""))), plot = last_plot(), width=7, height=5, dpi = 300, units="in")
